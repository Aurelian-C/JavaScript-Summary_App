    ...
    },
    {
      ...
      sectionArticles: [
        ...
        },
        {
          ...
          articleLinks: [{ linkTitle: '', linkSource: '' }],
          articleExplanations: [
            "When you run some JavaScript code in a browser, <b>the engine starts to parse the code. Each line is executed and popped on and off the call stack</b>. But, what about Web API's? <u>Web API's are not something JavaScript recognizes</u>, so the parser knows to pass it off to the browser for it to handle. When the browser has finished running its method, it puts what is needed to be ran by JavaScript into the <b>callback queue</b>. <b>The callback queue cannot be ran until the call stack is completely empty</b>. So, the <b>event loop</b> is constantly checking the call stack to see if it is empty so that it can add anything in the callback queue back into the call stack. And finally, once it is back in the call stack, it is ran and then popped off the stack.",
